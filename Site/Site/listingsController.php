...
function insertRow($jobTitle, $company, $location, $desc, $qual, $pay, $contactInfo){
    ...
    $sql = "INSERT INTO listing(uIDnum, job_title, job_description, qualifications, starting_pay, location, contactInfo, companyName) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $link->stmt_init();
    if ($stmt->prepare($sql)){
        $uIDnum = 2;
        $stmt->bind_param("isssssss", $uIDnum, $jobTitle, $desc, $qual, $pay, $location, $contactInfo, $company);
        $stmt->execute();
        $result = $stmt->get_result();
        printRow($jobTitle, $desc, $qual, $company, $location, $pay, $contactInfo, $uIDnum);
    } else {
...
function generateListings($query, $colQueried)
{
    ...
    if ($query == "") {
        ...
        <tr id="lastRow" style="height: 2em;">
            ...
            <td><div type= "submit" class="btn btn-success" id="submitButton">Submit</div></td>
